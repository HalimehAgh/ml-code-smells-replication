#!/usr/bin/env python3
"""
RQ2 Analysis - Main Runner
Executes all hypothesis test scripts in sequential order
"""
...
def main():
    ...
    print("="*80)
    print("RQ2 HYPOTHESIS TESTS - COMPLETE ANALYSIS")
    print("="*80)
    print(f"Analysis started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    
    # Define the scripts to run in order
    scripts = [
        ("H1", "part1_main_analysis.py"),
        ("H2", "part2_main_analysis.py"),
        ("H3", "part3_main_analysis.py"),
        ("H4", "part4_main_analysis.py"),
        ("H5", "part5_main_analysis.py"),
        ("H6", "part6_main_analysis.py"),
    ]
    
    # Track results
    results = {}
    base_path = Path(__file__).parent / "RQ2_tests"
    
    # Run each script
    for directory, script_name in scripts:
        script_path = base_path / directory / script_name
        ...
        # Run the script
        success = run_script(script_path, f"{directory}/{script_name}")
